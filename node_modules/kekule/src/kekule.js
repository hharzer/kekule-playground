...
// check if webpack
var isWebpack = !isNode && (typeof require !== 'undefined') && (typeof require === 'function');
...
{
	if (isNode || isWebpack)  // Node.js environment
	{
	...
	else  // in normal browser environment
	{
		if (!docReady && !forceDomLoader)  // can directly write to document
		{
			for (var i = 0, l = scriptUrls.length; i < l; ++i)
				directAppend(window.document, scriptUrls[i]);
			...
			window.document.write('<script type="text/javascript">' + sloadedCode + '<\/script>');
			if (callback)
				callback();
		}
		else {
			appendScriptFiles(window.document, scriptUrls, function() {
				// set a marker indicate that all modules are loaded
				Kekule._loaded();
				if (callback)
					callback();
...
function init()
{
	...
	else if ($root.document) { // browser not importing through webpack
		scriptInfo = analysisEntranceScriptSrc($root.document);
		scriptInfo.useMinFile = false;
	}
	else { // importing through webpack
		scriptInfo = {
			useMinFile: false,
			path: './'
		}
	...
	loadChildScriptFiles(scriptUrls, scriptInfo.forceDomLoader, function(){
		if (isNode || isWebpack)  // export Kekule namespace
		{
